...
for k in $m1; do
  ...
  for h in $(seq 1 $iteraciones); do
    ...
    printf "%d\n " $h
    $(dirname $0)/../../../ejercicio5-tipo1 < $(dirname $0)/grafosDeEntrada/grafo-n1-$n1-m1-$k-n2-$n2-m2-$m2.txt $cuantosVecinosMiro -t >> $(dirname $0)/tiempos-exp1-tipo1.txt
  done
...
for k in $m1; do
  ...
  for h in $(seq 1 $iteraciones); do
    echo "iteracion numero"
    printf "%d\n " $h
    $(dirname $0)/../../../ejercicio5-tipo2 < $(dirname $0)/grafosDeEntrada/grafo-n1-$n1-m1-$k-n2-$n2-m2-$m2.txt $cuantosVecinosMiro -t >> $(dirname $0)/tiempos-exp1-tipo2.txt
  done
  printf "\n" >> $(dirname $0)/tiempos-exp1-tipo2.txt
done

#hay dos archivos de tiempos, uno con los tiempos de "ejercicio5-tipo1" y otro con los tiempos de "ejercicio5-tipo1". Los graficos contienen en el eje x a m1 (que va variando) y en el eje y el tiempo de ejecucion correspondiente a cada m. Va a tener dos curvas, una para vecindad 1 y otra para vecindad 2. Además hay que graficar la complejidad. 
