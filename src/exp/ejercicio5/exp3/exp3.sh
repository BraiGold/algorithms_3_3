...
for k in $n; do
  ...
  for h in $(seq 1 $iteraciones); do
    ...
    printf "%d\n " $h
    $(dirname $0)/../../../ejercicio5-tipo1 < $(dirname $0)/grafo-n1-$k-m1-$m-n2-$k-m2-$m.txt -t >> $(dirname $0)/tiempos-exp3-tipo1.txt
  done
...
for k in $n; do
  ...
  for h in $(seq 1 $iteraciones); do
    echo "iteracion numero"
    printf "%d\n " $h
    $(dirname $0)/../../../ejercicio5-tipo2 < $(dirname $0)/grafo-n1-$k-m1-$m-n2-$k-m2-$m.txt -t >> $(dirname $0)/tiempos-exp3-tipo2.txt
  done
