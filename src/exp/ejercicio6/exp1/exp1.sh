...
for k in $m1; do
  ...
  for h in $(seq 1 $iteraciones); do
    ...
    printf "%d\n " $h
    $(dirname $0)/../../../ejercicio6-tipo1 < $(dirname $0)/grafosDeEntrada/grafo-n1-$n1-m1-$k-n2-$n2-m2-$m2.txt -t >> $(dirname $0)/tiempos-exp1-tipo1.txt
  done
...
for k in $m1; do
  ...
  for h in $(seq 1 $iteraciones); do
    echo "iteracion numero"
    printf "%d\n " $h
    $(dirname $0)/../../../ejercicio6-tipo2 < $(dirname $0)/grafosDeEntrada/grafo-n1-$n1-m1-$k-n2-$n2-m2-$m2.txt -t >> $(dirname $0)/tiempos-exp1-tipo2.txt
  done
