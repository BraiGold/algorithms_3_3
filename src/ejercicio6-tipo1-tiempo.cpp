...
vector<int> MCStabu(vector<int> mapeo, vector<vector<int> > grafoChico, vector<vector<int> > grafoGrande, int cuantosVecinosMiro, int maxTamTabu, int k, double cuantoDejoCorrer) {//k = No se encontro una mejora en las ultimas k iteraciones.
...
int main(int argc, char* argv[]) {
	...
	int k = atoi(argv[3]);
	double cuantoDejoCorrer = atoi(argv[4]);
	...
	if(n1 < n2) {
		mejorMapeo = MCStabu(mapeoInicial, grafo1, grafo2, cuantosVecinosMiro, maxTamTabu, k,cuantoDejoCorrer);
		resultado = calcularConjAristas(mejorMapeo, grafo1, grafo2);
		chico = n1;
	}else{
		mejorMapeo = MCStabu(mapeoInicial, grafo2, grafo1, cuantosVecinosMiro, maxTamTabu, k,cuantoDejoCorrer);
